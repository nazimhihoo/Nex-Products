        ...
        >
            ...
            >
                ...
                >
                    ...
                    <div class="nex-product-detail-shell container px-[60px] max-1180:px-0">
                        <div class="mt-12 flex gap-9 max-1180:flex-wrap max-lg:mt-0 max-sm:gap-y-4">
                            <!-- Gallery Blade Inclusion -->
                            @include('shop::products.view.gallery')

                            <!-- Details -->
                            <div class="relative max-w-[590px] rounded-[30px] border border-slate-200 bg-white p-7 shadow-[0_24px_70px_rgba(15,23,42,0.09)] max-1180:w-full max-1180:max-w-full max-1180:px-5 max-sm:px-4">
                                {!! view_render_event('bagisto.shop.products.name.before', ['product' => $product]) !!}

                                @if ($product->type == 'downloadable' || $product->type == 'virtual')
                                    <div class="mb-4 flex flex-wrap gap-2">
                                        <span class="nex-instant-label">Digital</span>
                                        <span class="nex-instant-label">Instant Access after payment</span>
                                    </div>
                                @endif

                                <div class="flex justify-between gap-4">
                                    <h1 class="break-words text-4xl font-extrabold leading-tight max-sm:text-2xl" v-pre>
                                        {{ $product->name }}
                                    </h1>
                                ...
                                <p class="mt-[22px] flex items-center gap-2.5 text-3xl !font-black max-sm:mt-2 max-sm:gap-x-2.5 max-sm:gap-y-0 max-sm:text-xl">
                                    {!! $product->getTypeInstance()->getPriceHtml() !!}
                                </p>

                                <p class="mt-3 inline-flex rounded-full px-3 py-1 text-sm font-bold {{ $product->isSaleable(1) ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700' }}">
                                    {{ $product->isSaleable(1) ? 'In stock and ready to order' : 'Currently out of stock' }}
                                </p>
                                ...
                                <div class="mt-8 flex max-w-[470px] gap-4 max-sm:mt-4 max-sm:flex-col">
                                    ...
                                    @if ($product->getTypeInstance()->showQuantityBox())
                                        <x-shop::quantity-changer
                                            name="quantity"
                                            value="1"
                                            class="gap-x-4 rounded-2xl border border-slate-200 bg-white px-7 py-4 max-md:py-3 max-sm:gap-x-5 max-sm:rounded-lg max-sm:px-4 max-sm:py-2"
                                        />
                                    @endif
                                ...
                                @if (core()->getConfigData('sales.checkout.shopping_cart.cart_page'))
                                    ...
                                    @if (core()->getConfigData('catalog.products.storefront.buy_now_button_display'))
                                        <x-shop::button
                                            type="submit"
                                            class="primary-button mt-5 w-full max-w-[470px] max-md:py-3 max-sm:mt-3 max-sm:rounded-lg max-sm:py-2"
        ...
        <script type="module">
            app.component('v-product', {
                ...
                methods: {
                    ...
                    checkWishlistStatus() {
                        const wishlistIndexUrl = @json(\Illuminate\Support\Facades\Route::has('shop.api.customers.account.wishlist.index') ? route('shop.api.customers.account.wishlist.index') : null);

                        if (this.isCustomer && wishlistIndexUrl) {
                            /**
                             ...
                             */
                            this.$axios.get(wishlistIndexUrl)
                                .then(response => {
                    ...
                    addToWishlist() {
                        const wishlistStoreUrl = @json(\Illuminate\Support\Facades\Route::has('shop.api.customers.account.wishlist.store') ? route('shop.api.customers.account.wishlist.store') : null);

                        if (this.isCustomer && wishlistStoreUrl) {
                            this.$axios.post(wishlistStoreUrl, {
