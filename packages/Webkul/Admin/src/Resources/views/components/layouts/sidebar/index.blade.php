@php
    $navigation = collect(config('nex_admin_navigation', []))->map(function (array $item) {
        $item['is_active'] = collect($item['active'] ?? [])->contains(fn (string $pattern) => request()->routeIs($pattern));
        $item['children'] = collect($item['children'] ?? [])->map(function (array $child) {
            $child['is_active'] = collect($child['active'] ?? [])->contains(fn (string $pattern) => request()->routeIs($pattern));
